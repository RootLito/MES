...
const List = () => {
    ...
    const handleDelete = async (id) => {
        try {
            const res = await axios.delete(`http://localhost:5000/survey/delete/${id}`);
            if (!res) {
                return (
                    <div className="toast toast-top toast-center">
                        <div className="alert alert-error text-white text-center">
            ...
            {deleted && (
                <div className="toast toast-top toast-center">
                    <div className="alert alert-success">
                        <span className="text-center text-green-50">Form deleted successfully</span>
                    </div>
                </div>
            )}
                            ...
                            surveys.map((survey) => (
                                ...
                                >
                                    ...
                                    <td className="flex gap-3 text-xl">
                                        <MdEditDocument className="text-primary cursor-pointer" onClick={() => navigate(`/update/${survey._id}`)} />
                                        <MdDeleteForever className="text-error cursor-pointer" onClick={() => openModal(survey._id)}
                                        />
